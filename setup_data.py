...
def load_study_registry() -> list:
    ...
    if actual_val != "MER Study ID":
        for i in range(1, 10):
            if str(ws.cell(row=i, column=1).value or "").strip() == "MER Study ID":
                HEADER_ROW = i
                break

    if str(ws.cell(row=HEADER_ROW, column=1).value or "").strip() != "MER Study ID":
        raise ValueError("'MER Study ID' header not found in Study Registry sheet")

    headers = [ws.cell(row=HEADER_ROW, column=c).value
               for c in range(1, ws.max_column + 1)]
    studies = []
    for row in ws.iter_rows(min_row=HEADER_ROW + 1, values_only=True):
        if not row[0] or not str(row[0]).startswith("MER"):
            continue
...
def main():
    ...
    print("  Key:   MER Study ID from Excel Study Registry")
    print("=" * 60)
    ...
    for study in studies:
        tax_id = str(study.get("MER Study ID") or "").strip()
        if not tax_id.startswith("MER"):
            continue
        ...
        printed = 0
        final_state = None
        for state in orchestrator.run(uploaded_file=FileWrapper(pdf_path)):
            for line in state["log"][printed:]:
                print(f"    {_clean_msg(line)}")
            printed = len(state["log"])
            final_state = state

        outcome = final_state.get("outcome") if final_state else "failed"
