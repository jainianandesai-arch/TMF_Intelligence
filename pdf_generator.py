...
Uses reportlab with Meridian Clinical brand colors.
"""
...
def header_footer(canvas, doc):
    ...
    canvas.drawString(1.5*cm, height - 1.1*cm, "Meridian Clinical")
    ...
    canvas.drawString(1.5*cm, 0.8*cm, f"Generated: {today}  ·  Meridian Clinical  ·  ICH E6 GCP Compliant")
...
def generate_executive_summary_pdf(summary_text, title, audience, today):
    ...
    doc = SimpleDocTemplate(
        ...
        author="Meridian Clinical TMF Intelligence System"
    )
    ...
    story.append(Paragraph(
        "CONFIDENTIAL — Meridian Clinical Internal Use Only · Not for external distribution",
        styles["footer"]
    ))
...
def generate_email_pdf(draft_text, study_name, document_name, today):
    ...
    story.append(Spacer(1, 1*cm))
    story.append(HRFlowable(width="100%", thickness=0.5, color=colors.HexColor("#E5E7EB"), spaceAfter=6))
    story.append(Paragraph("CONFIDENTIAL — Meridian Clinical Internal Use Only", styles["footer"]))
