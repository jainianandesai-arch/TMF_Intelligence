...
# ── Page config ──────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="TMF Intelligence | Meridian Clinical",
    page_icon="🧬",
    layout="wide",
...
if not st.session_state["entered"]:
    ...
    */
    ...
    .mc-cap .cap-icon { font-size: 1.2rem; margin-bottom: 0.4rem; }
    .mc-cap .cap-title {
        font-size: 0.82rem;
        font-weight: 700;
        color: #1B3A5C;
    ...
    <div class="mc-hero">
        <div class="mc-hero-left">
            <div class="mc-logo">Meridian<span> Clinical</span></div>
            <div class="mc-eyebrow">AI-Powered Internal Tool</div>
            <div class="mc-headline">TMF<br><span>Intelligence</span><br>System</div>
            <div class="mc-tagline">Smarter trial document management. Powered by Claude AI.</div>
            <div class="mc-stats">
                <div class="mc-stat">
                ...
                <div class="mc-stat">
                    <div class="n accent">{total_critical}</div>
                    <div class="l">Critical Flags</div>
                </div>
                <div class="mc-stat">
                    <div class="n accent">{total_warning}</div>
    ...
    st.markdown("""
    <div class="mc-body">
        <div class="mc-section-title">Platform Capabilities</div>
        <div class="mc-capabilities">
            <div class="mc-cap">
    ...
    <div class="mc-footer-bar">
        <span>Meridian Clinical Internal Tool &nbsp;·&nbsp; Not for external distribution</span>
...
with st.sidebar:
    ...
    module = st.radio(
        "Navigate",
        ["📊 Study Overview", "📁 TMF Tracker", "🚨 Flags Dashboard", "💬 Query Studies", "✉️ Draft Communications", "📋 Executive Summary", "📥 Ingestion Tracker", "🕸️ Live Ingestion Demo"],
...
<div class="tmf-header">
    ...
    <p>GI Clinical Trials Portfolio — Meridian Clinical Internal</p>
...
if module == "📊 Study Overview":
    ...
    if not filtered:
        ...
        with left_col:
            ...
            .chev-wrap div[data-testid="stButton"] > button:hover { background:#0D9488 !important; }
            </style>
            """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if ask_clicked and query.strip():
        with st.spinner("Claude is analyzing the trial intelligence system..."):
            ...
            system_prompt = f"""You are a clinical trial intelligence assistant for Meridian Clinical.
You have access to the structured TMF portfolio database below.

CRITICAL IDENTIFIER RULES:
- MER Study ID (e.g. MER-2026-001) is the internal Meridian Clinical registry key.
...
- Do NOT confuse MER Study ID with Protocol Number — they are different fields.
- When asked for a protocol number, return the Protocol Number field, not the MER Study ID.
- Always cite the study name, MER Study ID, and protocol number when available.
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    def clean_sender_signature(name: str, email: str) -> str:
        ...
        if name and email:
            return f"{name}, Meridian Clinical TMF Associate ({email})"
        if name:
            return f"{name}, Meridian Clinical TMF Associate"
        if email:
            return f"Meridian Clinical TMF Associate ({email})"
        return "Meridian Clinical TMF Associate"
    ...
    if not flagged_docs:
        ...
        if st.button("Draft Communication →", type="primary"):
            with st.spinner("Drafting..."):
                ...
                prompt = f"""Draft a controlled single-study TMF follow-up email for a Meridian Clinical TMF Associate.
...
Subject: TMF Document Request – Investigator Agreement | QUASAR (MER-2026-001)
Dear Site Coordinator,
I am following up regarding the Investigator Agreement for QUASAR (MER-2026-001), which is currently missing from the Trial Master File. Please provide the fully executed document or confirm that it has been uploaded to the TMF within five business days. This document is required to evidence investigator agreement to conduct the study in accordance with the protocol and applicable requirements.
...
- Keep the subject concise: document name, study name, and MER Study ID.
...
"""
    ...
    if st.button("Generate Executive Summary →", type="primary"):
        with st.spinner("Claude is generating your summary..."):
            ...
            prompt = f"""Generate a controlled executive summary for Meridian Clinical as of {today}.
...
- Use MER Study ID and Protocol Number correctly; do not confuse them.
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("How to process new documents", expanded=False):
        st.markdown("""
**Governance rule:** the pipeline can identify, validate, route, and index documents — but it cannot create a MER Study ID or update the Excel registry.
...
<div style="background:#EFF6FF;border:1px solid #BFDBFE;border-left:4px solid #1A3F6F;border-radius:8px;padding:0.9rem 1.2rem;margin-bottom:0.6rem;">
  ...
  <div style="font-size:0.75rem;color:#9CA3AF;margin-top:0.2rem;">{_queue} · Modified: {_mtime}</div>
    ...
    st.markdown('<div class="section-header">Document Lifecycle Timeline</div>', unsafe_allow_html=True)
    st.caption(
        "Full history per file, reconstructed from the audit log — the clearest evidence that "
        "quarantine correctly held a file until the registry caught up, then released it."
    )
    if _audit_rows:
        _by_file = {}
        for _r in _audit_rows:
            _name = _r.get("source_file", "") or "(unknown file)"
            _by_file.setdefault(_name, []).append(_r)

        _selected_file = st.selectbox(
            "Select a file to view its lifecycle",
            sorted(_by_file.keys()),
            key="lifecycle_file_select",
        )
        if _selected_file:
            _events = sorted(_by_file[_selected_file], key=lambda r: r.get("timestamp", ""))
            for _e in _events:
                _status = _action(_e)
                _bg, _fg = _STATUS_COLOR.get(_status, ("#F3F4F6", "#374151"))
                _detail = _e.get("error") or _e.get("final_location") or "No additional detail recorded"
                st.markdown(f"""
<div style="display:flex;gap:0.75rem;margin-bottom:0.5rem;align-items:flex-start;">
  <div style="min-width:150px;font-size:0.78rem;color:#6B7280;padding-top:0.5rem;">{_safe(_e.get('timestamp'))}</div>
  <div style="flex:1;background:{_bg};border-left:4px solid {_fg};border-radius:6px;padding:0.6rem 0.9rem;">
    <span style="font-weight:600;color:{_fg};">{_status}</span>
    <span style="font-size:0.75rem;color:#374151;"> &mdash; {_safe(_detail)}</span>
  </div>
</div>""", unsafe_allow_html=True)
    else:
        st.info("No audit records yet. Run ingest_protocol.py or the Live Ingestion Demo tab to start tracking.")
...
elif module == "🕸️ Live Ingestion Demo":
    import sys as _sys2
    from pathlib import Path as _Path3

    _proj_root = _Path3(__file__).parent
    if str(_proj_root) not in _sys2.path:
        _sys2.path.insert(0, str(_proj_root))
    from agents import TMFOrchestrator, build_ingestion_graph

    st.markdown("### 🕸️ Live Ingestion Demo")
    st.markdown(
        '<p style="color:#6B7280;font-size:0.85rem;margin-top:-0.5rem;">'
        'Upload a protocol PDF and watch it move through the LangGraph pipeline in real time. '
        'This tab invokes the pipeline directly for demonstration — normally ingestion runs as a '
        'backend process (see TMF_Ingestion_Agent/ingest_protocol.py).</p>',
        unsafe_allow_html=True
    )
    st.markdown("---")
    ...
    st.markdown('<div class="section-header">Pipeline Graph Structure</div>', unsafe_allow_html=True)
    st.caption("Rendered directly from the compiled LangGraph StateGraph — the dashed edges are the conditional branches.")
    if "live_demo_graph_png" not in st.session_state:
        try:
            _graph = build_ingestion_graph()
            st.session_state["live_demo_graph_png"] = _graph.get_graph().draw_mermaid_png()
            st.session_state["live_demo_graph_mermaid"] = _graph.get_graph().draw_mermaid()
        except Exception as _e:
            st.session_state["live_demo_graph_png"] = None
            st.session_state["live_demo_graph_mermaid"] = str(_e)

    if st.session_state.get("live_demo_graph_png"):
        st.image(st.session_state["live_demo_graph_png"])
    else:
        st.warning("Could not render the graph as an image (Mermaid rendering needs network access). Showing the raw graph definition instead:")
        st.code(st.session_state.get("live_demo_graph_mermaid", ""), language="text")

    st.markdown("<br>", unsafe_allow_html=True)

    # ── Upload + run ───────────────────────────────────────────────────────────
    st.markdown('<div class="section-header">Upload a Protocol</div>', unsafe_allow_html=True)
    _uploaded = st.file_uploader("Drop a protocol PDF", type=["pdf"], key="live_demo_uploader")
    _run_clicked = st.button("▶ Run through pipeline", type="primary", disabled=_uploaded is None)

    _steps_placeholder = st.empty()
    _detail_placeholder = st.empty()

    def _render_steps(active_node, outcome):
        _cols = _steps_placeholder.columns(len(_NODE_ORDER))
        _order = [n[0] for n in _NODE_ORDER]
        _active_idx = _order.index(active_node) if active_node in _order else -1
        for _i, (_key, _icon, _label) in enumerate(_NODE_ORDER):
            if outcome in ("duplicate", "unregistered", "failed") and _i > _active_idx:
                _bg, _fg, _border, _state_text = "#F9FAFB", "#9CA3AF", "1px dashed #D1D5DB", "not reached"
            elif _i < _active_idx or (_i == _active_idx and outcome == "success"):
                _bg, _fg, _border, _state_text = "#D1FAE5", "#059669", "1px solid #6EE7B7", "done"
            elif _i == _active_idx:
                _bg, _fg, _border, _state_text = "#0D9488", "white", "1px solid #0D9488", "active"
            else:
                _bg, _fg, _border, _state_text = "#F3F4F6", "#6B7280", "1px solid #E5E7EB", "pending"
            _cols[_i].markdown(f"""
<div style="background:{_bg};color:{_fg};border:{_border};border-radius:8px;padding:0.6rem;text-align:center;">
  <div style="font-size:0.95rem;">{_icon}</div>
  <div style="font-size:0.78rem;font-weight:600;">{_label}</div>
