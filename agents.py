...
Primary Key: MER Study ID (from eTMF Excel — never generated here)
...
File naming (registered only):
  MER-ID_StudyName_PrimaryRef_DocType_Version_Date.pdf
  e.g. MER-2026-005_ADVANCE_M16-006_Amendment_Amendment-2.0_2022-08-04.pdf
...
def load_excel_registry() -> list:
    ...
    try:
        ...
        if actual_val != "MER Study ID":
            for i, row in enumerate(rows_list[:10]):
                if str(row[0].value or "").strip() == "MER Study ID":
                    HEADER_ROW = i + 1
                    break
...
def build_pdf_filename(tax_id: str, study_name: str, identifiers: dict,
    ...
    Format: MER-ID_StudyName_PrimaryRef_DocType_Version_Date.pdf
    """
...
# MER Study ID comes from Excel only — never generated here.
...
class RegistryAgent:
    def run(self, metadata: dict, original_filename: str = None) -> dict:
        ...
        # ── Registry match found — MER ID from Excel only ──────────────────────
        tax_id     = str(excel_row.get("MER Study ID") or "").strip()
        study_name = str(excel_row.get("Study Name") or metadata.get("short_name") or "").strip()

        if not tax_id:
            return {"ok": False, "message": "MER Study ID missing from Excel registry row."}
        ...
        excel_ref = str(excel_row.get("Protocol No") or "").strip()
        ...
        def _py_literal(v):
            """Like json.dumps, but emits Python's None instead of JSON's null."""
    ...
    "{tax_id}": {{
        ...
        "protocol_date": {_py_literal(protocol_date)},
        "latest_amendment": "{latest_amendment}",
        "latest_amendment_date": {_py_literal(latest_amendment_date)},
    ...
    def _quarantine(self, original_filename: str):
        """Move file from incoming/ to unregistered/. Never rename with MER ID."""
...
class IndexingAgent:
    ...
    def run(self, full_text: str, metadata: dict, progress_callback=None) -> dict:
        ...
        # Remove existing chunks for this MER ID
        try:
            existing = collection.get(where={"tax_id": tax_id})["ids"]
            if existing:
...
#
# Nodes: upload -> metadata -> registry -> {indexing -> sync} | END
# Registry is the gatekeeper: duplicate/unregistered routes end the run early
# (outcome recorded in state, not a magic sentinel string). Any node that
# fails to complete also routes straight to END with outcome="failed".
# ══════════════════════════════════════════════════════════════════════════════

class IngestionState(TypedDict):
    # Inputs — exactly one of these is set per run
    uploaded_file: Any
    url: Optional[str]
    inbox_path: Optional[str]
    ...
    outcome: Optional[str]                     # None while running; else success/duplicate/unregistered/failed
    log: Annotated[List[str], operator.add]     # accumulates across the run


def upload_node(state: IngestionState) -> dict:
    result = UploadAgent().run(
        uploaded_file=state.get("uploaded_file"),
        url=state.get("url"),
        inbox_path=state.get("inbox_path"),
    )
    if not result["ok"]:
        return {"node": "upload", "outcome": "failed",
                "log": [f"Upload Agent failed: {result['message']}"]}
    return {
        "node": "upload",
        "pdf_bytes": result["pdf_bytes"],
        "original_filename": result.get("original_filename", "protocol.pdf"),
        "log": [f"Upload Agent: {result['message']}"],
    }


def route_after_upload(state: IngestionState) -> str:
    return END if state.get("outcome") == "failed" else "metadata"


def metadata_node(state: IngestionState) -> dict:
    result = MetadataAgent().run(state["pdf_bytes"])
    if not result["ok"]:
        return {"node": "metadata", "outcome": "failed",
                "log": [f"Metadata Agent failed: {result['message']}"]}
    log = [f"Metadata Agent: {result['message']}"]
    if not result.get("has_identifier"):
        log.append("Metadata Agent warning: no standard identifier found — "
                    "will attempt Excel lookup by study name.")
    return {
        "node": "metadata",
...
def route_after_metadata(state: IngestionState) -> str:
    return END if state.get("outcome") == "failed" else "registry"


def registry_node(state: IngestionState) -> dict:
    result = RegistryAgent().run(state["metadata"], state.get("original_filename"))

    if result.get("duplicate"):
        return {"node": "registry", "outcome": "duplicate", "tax_id": result["tax_id"],
                "log": [f"Registry Agent: {result['message']}"]}

    if result.get("unregistered"):
        return {"node": "registry", "outcome": "unregistered",
                "log": [f"Registry Agent: unregistered — {result['message']}"]}

    if not result["ok"]:
        return {"node": "registry", "outcome": "failed",
                "log": [f"Registry Agent failed: {result['message']}"]}
...
def route_after_registry(state: IngestionState) -> str:
    if state.get("outcome") in ("duplicate", "unregistered", "failed"):
        return END
    return "indexing"
...
def sync_node(state: IngestionState) -> dict:
    ...
    return {
        ...
        "outcome": "success",
        "log": [
            "Portfolio Sync Agent: dashboard cache cleared",
            f"{metadata.get('drug', '?')} ({state.get('tax_id', '?')}) "
...
class TMFOrchestrator:
    ...
    run() streams the full state dict after each node completes, so callers
    (CLI, Streamlit) can show live progress. The final yielded state's
    "outcome" is one of: "success", "duplicate", "unregistered", "failed".
    """
    def __init__(self):
