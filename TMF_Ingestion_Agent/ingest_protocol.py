...
def _clean_msg(msg):
    """Replace common Unicode punctuation, then strip remaining non-ASCII for Windows console output."""
    for old, new in [("—", "--"), ("→", "->")]:
...
def run_pipeline(pdf_path: Path = None, url: str = None) -> str:
    """Run one file through the LangGraph pipeline. Returns 'ingested', 'duplicate', 'unregistered', or 'failed'."""
    orchestrator = TMFOrchestrator()

    if url:
        stream = orchestrator.run(url=url)
    else:
        stream = orchestrator.run(inbox_path=str(pdf_path))

    printed = 0
    final_state = None
    for state in stream:
        for line in state["log"][printed:]:
            print(_clean_msg(line))
        printed = len(state["log"])
        final_state = state

    outcome = final_state.get("outcome") if final_state else "failed"
    return _OUTCOME_LABELS.get(outcome, "failed")
...
if SINGLE_ARG:
    ...
    print_header("TMF INGESTION AGENT  |  Meridian Clinical")
...
print_header("TMF INGESTION AGENT  |  Meridian Clinical  |  Auto Mode")
